...
use Hash::MultiValue;
use Stream::Buffered;
use Ark::BodyParser::OctetStream;

# copied Kossy::BodyParser::*
...
sub get_parser {
    ...
    return Ark::BodyParser::OctetStream->new();
}
...
sub parse {
    ...
    if (!$ct && !$cl) {
        ...
        $env->{'ark.request.body_parameters'}   = [];
        $env->{'ark.request.upload_parameters'} = [];
        return;
    }
    ...
    ($env->{'ark.request.body_parameters'}, $env->{'ark.request.upload_parameters'})
        = $parser->finalize();
